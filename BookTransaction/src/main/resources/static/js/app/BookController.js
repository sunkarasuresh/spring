...
angular.module('bookApp').controller('BookController',
    ['BookService', '$scope',  function( BookService, $scope) {
            ...
            BookService.updateBook(Book, id)
                .then(
                    ...
                    function(errResponse){
                        ...
                        self.errorMessage='Error while updating Book '+errResponse.data.errorMessage;
                        self.successMessage='';
                    }
                );
        }
        ...
        {
        	 console.log('About to update Book stock');
             BookService.countUpdate(id,txType)
                 .then(
                     function (response){
                         console.log('Book updated successfully');
                         self.txMessage='Book updated successfully';
                         self.txErrorMessage='';
                         self.done = true;
                         $scope.myForm.$setPristine();
                     },
        ...
        function removeBook(id){
            ...
            BookService.removeBook(id)
                .then(
                    ...
                    function(errResponse){
                        console.error('Error while removing Book '+id +', Error :'+errResponse.data.errorMessage);
                    }
                );
        }
